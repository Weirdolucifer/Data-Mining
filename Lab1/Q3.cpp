...
vector<vector<int> > data_set1;
vector<vector<int> > data_set2;
vector<vector<int> > new_data;
int supp[100000];
int flag[100000];
int newsupp[100000];
void apriori(vector<vector<int> > data,vector<vector<int> > data_set)
{
    if(n == 0)
    return;
...
void run(map<int, int> mp,vector<vector<int> > data_set)
{
...
int main()
{
    ...
    for(int i=0;i<data_set.size();i++)
    {
    	if(i<n/2)
    	{
    		data_set1.push_back(data_set[i]);
    	...
    	{
    		data_set2.push_back(data_set[i]);
    		for(int j=0;j<data_set[i].size();j++)
    		{
    			mp2[data_set[i][j]]++;
			mp[data_set[i][j]]++;
